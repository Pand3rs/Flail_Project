...
namespace My_Game
{
	...
	void init(int screen_w, int screen_h)
	{
		...
		Particle::init(&World::flail_link, "dirt.txt", 4096, Engine::renderer);
...
#pragma region ENEMY
		{
			...
			for (int i = 0; i < World::enemy.array_size; i++)
			{
					...
					for (int x = enemy_imprint_region.first_col; x <= enemy_imprint_region.last_col; x++)
					{
						...
						if (e != -1 && e != i)
						{
							...
							if (r == 1)
							{
								...
								Particle::spawn(&World::flail_link, 1, 0.2, &pos, &initial_vel, &f_min, &f_max, 1000, 2000, current_time);
				...
				Particle::spawn(&World::flail_link, 1, p_size, &epos, &initial_vel, &f_min, &f_max, 500, 1000, current_time);
					...
					for (int x = region.first_col; x <= region.last_col; x++)
					{
						...
						if (Grid::get_Tile(x, y, &World::imprint) != -1)
						{
							...
							if (r == 1)
							{
								...
								Particle::spawn(&World::flail_link, 1, 0.125, &pos, &initial_vel, &f_min, &f_max, 500, 2000, current_time);
								Actor::destroy(i, &World::bullet);
