...
namespace Particle
{
	...
	int is_Spawned(int which_particle, const Emitter *f);

	Shape::Rect::Data *get_World_Coord(int which_particle, Emitter *e);

	Vec2D::Vec2D *get_Pos(int which_particle, Emitter *e);
	Vec2D::Vec2D *get_Vel(int which_particle, Emitter *e);

	void update_Vel_and_Life(Emitter *e, unsigned int current_time, float dt);

	void add_Force(int which_particle, Emitter *e, Vec2D::Vec2D *force);
...
namespace Particle
{
	...
	{
		return e->spawn_stack.spawned[which_particle];
	}

	Shape::Rect::Data *get_World_Coord(int which_particle, Emitter *e)
	{
		return &e->world_coords.rect[which_particle];
	}

	Vec2D::Vec2D *get_Pos(int which_particle, Emitter *e)
	{
		return &e->bodies.pos[which_particle];
	}
	Vec2D::Vec2D *get_Vel(int which_particle, Emitter *e)
	{
		return &e->bodies.vel[which_particle];
	}
	...
	void add_Force(int which_particle, Emitter *e, Vec2D::Vec2D *force)
	{
		Body::add_Force(which_particle, &e->bodies, force);
	}
